...
Flows:
  * top_up: learner buys credits
  * enroll: learner prepays a full course
  * deliver_session: accrue tutor pay and recognize revenue
  * payout_tutor: pay tutors from accrued balances
"""
...
from .models import (
    Cohort,
    CohortStatus,
    LEARNER_FEE_PER_HOUR_CENTS,
    Session,
    SessionStatus,
    TUTOR_PAY_PER_HOUR_CENTS,
    Tutor,
)
    ...
    def enroll(
        ...
        entry = self._l.post(
            JournalEntry(
                ...
                memo=(
                    f"enroll cohort={cohort.id} "
                    f"{course_hours}h @ "
                    f"${LEARNER_FEE_PER_HOUR_CENTS / 100}/h "
                    f"= ${cost / 100}"
                ),
            )
        )
        ...
        entry = self._l.post(
            JournalEntry(
                ...
                memo=(
                    ...
                    f"{attendees} learners x {hours}h "
                    f"-> fees ${learner_fees / 100}, "
                    f"tutor ${tutor_pay / 100}, "
                    f"spread ${spread / 100}"
                ),
            )
        )
        ...
        return self._l.post(
            JournalEntry(
                ...
                postings=(
                    ...
                    Posting(tutor_paid(tutor.id), +amount),
